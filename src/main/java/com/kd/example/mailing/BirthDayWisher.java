package com.kd.example.mailing;
...
import com.kd.example.mailing.bwisher.beans.config.QuartzProperties;
import com.kd.example.mailing.bwisher.quartz.EmailSenderJob;

@SpringBootApplication
@ComponentScan(basePackages = { "com.kd.example.mailing" })
public class BirthDayWisher {
	...
	private static void ScheduleJob(QuartzProperties quartzProperties) throws SchedulerException, InterruptedException {
		...
		JobDetail job = JobBuilder.newJob(EmailSenderJob.class)
				.withIdentity(quartzProperties.getJobName(), quartzProperties.getGroupName()).build();
		Trigger trigger = TriggerBuilder.newTrigger()
				.withIdentity(quartzProperties.getTriggerName(), quartzProperties.getGroupName())
				.withSchedule(CronScheduleBuilder.cronSchedule(quartzProperties.getCronExpression()))
